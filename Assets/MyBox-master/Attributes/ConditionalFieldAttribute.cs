...
namespace MyBox.Internal
{
	...
	public class ConditionalFieldAttributeDrawer : PropertyDrawer
	{
		...
		private void CachePropertyDrawer(SerializedProperty property)
		{
			...
			if (customDrawer == null) customDrawer = TryGetUnityEventAttributeDrawer();

			if (customDrawer == null) customDrawer = TryCreateAttributeDrawer();

			_customPropertyDrawer = customDrawer;

			// EDIT: Try to get drawer for base type (for objects that inherit from UnityEvent)
			PropertyDrawer TryGetUnityEventAttributeDrawer()
            {
				var attributeBaseType = typeof(UnityEvent);
				var baseDrawerType = CustomDrawerUtility.GetPropertyDrawerTypeForFieldType(attributeBaseType);
				if (baseDrawerType == null) return null;
				
				return CustomDrawerUtility.InstantiatePropertyDrawer(baseDrawerType, fieldInfo, attribute);
			}
